...
#### Authentication
We will use Auth0, a  for login. The flow of authentication is shown below. The user will login with some form of social account, such as Google, and will get authenticated by Auth0. They will then be granted an access_token (A JSON Web Token which can be also used to specify the scope of the user's permissions), and their information will be used to get the list of their servers from our database. 
![Website Design](images/auth.png  "Website Control Concept")
#### Database
We have our own PostgreSQl database, used to store the list of servers for each user.
